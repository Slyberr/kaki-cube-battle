  ...
  <UPageHero title="Bienvenue sur Kaki Cube VS !"
    ...
    <template v-if="displayHomePage">
      <UModal title="Créer une salle">
        ...
        <template #body>
          <div class="text-sm" :class="windowWidth < 1000 ? 'flex' : 'hidden'">Pour les appareils mobiles et tablette,
            Kaki
                  ...
                  <UModal class="px-3" :title="`Rejoindre la salle ${room.roomname}`">
                    ...
                    <template #body>
                      <UForm :schema="schemaJoin" :state="stateJoin" class="m-8 space-y-4"
                        ...
                        <div class="text-sm" :class="windowWidth < 1000 ? 'flex' : 'hidden'">Pour les appareils mobiles
                          et
    ...
    <template v-else>
      <p class="text-xl xl:text-2xl text-center">Vous êtes déjà dans une salle !</p>
      <i>
        <p class="text-md xl:text-lg text-center">Vérifiez vos onglets ouvert sur la navigateur. Si aucun n'est ouvert, rechargez la page pour retrouver
          votre salle et si vous l'avez volontairement quitté, en rejoindre/créer une.
          <br/>
          <br/>
          Au passage, le délai avant de plus pouvoir rejoindre la salle après avoir fermé un onglet est
          de 30 à 40 minutes.
        </p>
      </i>
